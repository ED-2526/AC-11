...
def split_train_test(method, test_size=0.2, random_state=42):
    if os.path.exists(os.path.join(os.path.dirname(__file__), f'train_data_{method}.pickle')):
...
def build_vocabulary(train_dict, K=100):
    all_descriptors = []
...
def vocabulary(method = 'sift'):
    train_data, test_data = split_train_test(method)
    kmeans = build_vocabulary(train_data)
